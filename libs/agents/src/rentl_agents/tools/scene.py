...
def scene_read_metadata(context: ProjectContext, scene_id: str) -> str:
    """Return metadata for the scene without transcript."""
    logger.info("Tool call: scene_read_metadata(scene_id=%s)", scene_id)
    ...
    meta = [
        ...
        f"Summary: {scene.annotations.summary or '(not set)'}",
    ]
    return "\n".join(meta)


async def scene_read_overview(context: ProjectContext, scene_id: str) -> str:
    """Return metadata and transcript for the scene (summary included if present)."""
    logger.info("Tool call: scene_read_overview(scene_id=%s)", scene_id)
    ...
    meta = [
        ...
        f"Primary Characters: {', '.join(scene.annotations.primary_characters)}",
        f"Locations: {', '.join(scene.annotations.locations)}",
        "Summary:" if scene.annotations.summary else "Summary not yet recorded.",
    ]
    if scene.annotations.summary:
        meta.append(scene.annotations.summary)
    meta.extend(["Transcript:", transcript])
    return "\n".join(meta)


async def scene_read_redacted(context: ProjectContext, scene_id: str) -> str:
    """Return metadata and transcript for the scene with summaries redacted."""
    logger.info("Tool call: scene_read_redacted(scene_id=%s)", scene_id)
    lines = await context.load_scene_lines(scene_id)
    scene = context.get_scene(scene_id)
    ...
    meta = [
        ...
        f"Primary Characters: {', '.join(scene.annotations.primary_characters)}",
        f"Locations: {', '.join(scene.annotations.locations)}",
        "Existing summary redacted (provide your own).",
        "Transcript:",
        transcript,
    ]
    return "\n".join(meta)
...
async def scene_update_summary(
...
async def scene_update_tags(context: ProjectContext, scene_id: str, tags: list[str], *, written_tags: set[str]) -> str:
...
async def scene_update_primary_characters(
...
async def scene_update_locations(
