...
Address action items from an audit that were marked "Fix Now". Reads `audit.md`, fixes each issue, and verifies with `make all`.
...
Ask only if an item is ambiguous (use AskUserQuestion).
