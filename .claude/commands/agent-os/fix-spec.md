...
Address action items from an audit that were marked "Add to Current Spec (Fix Now)". This command reads the audit.md, fixes each issue at the cited location, and verifies the fix with `make all`.
...
- **If unsure, ask** — If an action item is ambiguous, use AskUserQuestion to clarify before making changes.
