...
- [x] Task 9: Recombine pretranslation quality test into single scenario (Signpost 4 — REDO)
  ...
  - Files: `tests/quality/agents/test_pretranslation_agent.py`, `tests/quality/features/agents/pretranslation_agent.feature`
  - Acceptance: `make quality` passes; pretranslation quality test is a single scenario that runs the real agent AND has the LLM judge evaluate the real output
