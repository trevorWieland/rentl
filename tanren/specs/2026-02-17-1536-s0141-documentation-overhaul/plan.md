...
- [x] Task 5: Write Data Schema reference
  ...
  - [x] Fix: Correct stale artifact-storage docs that claim named phase files under `.rentl/artifacts/{run_id}/`; runtime persists `artifact-{artifact_id}.jsonl` with a global `.rentl/artifacts/index.jsonl` (`docs/data-schemas.md:226`, `docs/data-schemas.md:443`, `packages/rentl-io/src/rentl_io/storage/filesystem.py:202`, `packages/rentl-io/src/rentl_io/storage/filesystem.py:517`, `packages/rentl-core/src/rentl_core/orchestrator.py:1337`) (audit round 2)
...
- [x] Task 7: Stabilize full verification quality gate
  ...
  - [x] Fix: Make the unit coverage gate fail hard when coverage is below 80%; current gate command logs `FAIL Required test coverage of 80% not reached. Total coverage: 79.82%` but still exits success (`Makefile:69`, `pyproject.toml:84`) (audit round 4)
