"""Live LLM agentevals-style check for the translation reviewer subagent."""
...
from rentl_agents.subagents.translation_reviewer import (
    build_translation_reviewer_user_prompt,
    create_translation_reviewer_subagent,
)
...
async def test_translation_reviewer_live_calls_and_language(tiny_vn_tmp: Path, llm_judge_model: str) -> None:
    """Translation reviewer should read translations and record review checks."""
    context = await load_project_context(tiny_vn_tmp)
    scene_id = "scene_a_00"

    prompt = build_translation_reviewer_user_prompt(scene_id)
    subagent = create_translation_reviewer_subagent(
        context,
    ...
    result = await run_agent_with_auto_approve(
        ...
        thread_id="llm-live:translation-reviewer:scene_a_00",
    )

    messages = result.get("messages")
    assert isinstance(messages, list), "Expected messages list from translation reviewer run"
    assert messages, "Expected non-empty messages from translation reviewer run"
    msg_list = cast(list[BaseMessage], messages)
    tool_names = _extract_tool_call_names(msg_list)
    required_tools = {"read_translations", "record_translation_review"}
    ...
    assert translations, "Expected translations to exist for review checks"
    assert any("translation_review" in t.meta.checks for t in translations), "Expected translation reviews recorded"
    ...
    if not eval_result.get("score"):
        ...
        pytest.fail(f"LLM judge failed translation_reviewer trajectory: {reason}")
