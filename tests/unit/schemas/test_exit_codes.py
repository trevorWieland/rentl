...
def test_all_orchestration_codes_mapped() -> None:
    ...
    for member in OrchestrationErrorCode:
        ...
        assert key in ERROR_CODE_TO_EXIT_CODE, (
            f"OrchestrationErrorCode.{member.name} "
            f"({key}) missing from registry"
        )
        assert (
            ERROR_CODE_TO_EXIT_CODE[key]
...
def test_all_ingest_codes_mapped() -> None:
    ...
    for member in IngestErrorCode:
        ...
        assert key in ERROR_CODE_TO_EXIT_CODE, (
            f"IngestErrorCode.{member.name} "
            f"({key}) missing from registry"
        )
        assert (
            ERROR_CODE_TO_EXIT_CODE[key]
            == ExitCode.INGEST_ERROR
        )
...
def test_all_storage_codes_mapped() -> None:
    ...
    for member in StorageErrorCode:
        ...
        assert key in ERROR_CODE_TO_EXIT_CODE, (
            f"StorageErrorCode.{member.name} "
            f"({key}) missing from registry"
        )
        assert (
            ERROR_CODE_TO_EXIT_CODE[key]
...
def test_cli_level_codes_present() -> None:
    ...
    assert (
        ERROR_CODE_TO_EXIT_CODE["config_error"]
    ...
    assert (
        ERROR_CODE_TO_EXIT_CODE["runtime_error"]
...
def test_resolve_cli_validation_error_without_domain() -> None:
    ...
    assert (
        resolve_exit_code("validation_error")
        == ExitCode.VALIDATION_ERROR
    )
...
def test_resolve_domain_qualified_ingest() -> None:
    ...
    assert (
        resolve_exit_code("validation_error", domain="ingest")
        == ExitCode.INGEST_ERROR
    )
...
def test_resolve_domain_qualified_export() -> None:
    ...
    assert (
        resolve_exit_code("io_error", domain="export")
        == ExitCode.EXPORT_ERROR
    )
...
def test_resolve_domain_qualified_storage() -> None:
    ...
    assert (
        resolve_exit_code("not_found", domain="storage")
        == ExitCode.STORAGE_ERROR
    )
...
def test_resolve_domain_qualified_orchestration() -> None:
    ...
    assert (
        resolve_exit_code(
            "missing_dependency", domain="orchestration"
        )
        == ExitCode.ORCHESTRATION_ERROR
    )
...
def test_resolve_unknown_code_returns_runtime_error() -> None:
    ...
    assert (
        resolve_exit_code("totally_unknown")
        == ExitCode.RUNTIME_ERROR
    )
...
def test_resolve_unknown_domain_falls_back() -> None:
    ...
    assert (
        resolve_exit_code("config_error", domain="nonexistent")
        == ExitCode.CONFIG_ERROR
    )
...
def test_resolve_unknown_domain_and_code() -> None:
    ...
    assert (
        resolve_exit_code(
            "unknown_code", domain="unknown_domain"
        )
        == ExitCode.RUNTIME_ERROR
    )
