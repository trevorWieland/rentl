...
#
# Spins up parallel codex agents to audit every standard in agent-os/standards/.
# Each agent reads the standard + codebase and writes a structured report.
#
# Usage:
#   ./agent-os/scripts/audit-standards.sh
#   ./agent-os/scripts/audit-standards.sh --concurrency 5
#   ./agent-os/scripts/audit-standards.sh --config audit.conf
#   ./agent-os/scripts/audit-standards.sh --standards async-first-design,naming-conventions
#
...
#   AUDIT_STANDARDS_DIR - Path to standards directory (default: "agent-os/standards")
#   AUDIT_OUTPUT_DIR    - Output directory (default: "agent-os/audits/YYYY-MM-DD")
...
STANDARDS_DIR="${AUDIT_STANDARDS_DIR:-agent-os/standards}"
TODAY=$(date +%Y-%m-%d)
OUTPUT_DIR="${AUDIT_OUTPUT_DIR:-agent-os/audits/$TODAY}"
...
TEMPLATE_FILE="agent-os/audits/standard-audit-template.md"
