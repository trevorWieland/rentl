...
**Spec List (Expanded):**
- ✅ (v0.1.01) Schema Definitions & Validation — Define strict Pydantic schemas for configs, inputs, outputs, and artifacts so the pipeline is deterministic and debuggable. **Depends on:** None.
- ✅ (v0.1.02) Progress Semantics & Tracking — Establish progress invariants and summaries so phases can report trustworthy status and completion. **Depends on:** v0.1.01.
- ✅ (v0.1.03) Import Adapter: CSV/JSONL/TXT — Normalize input data into canonical `SourceLine` records with schema validation. **Depends on:** v0.1.01.
- ✅ (v0.1.04) Export Adapter: CSV/JSONL/TXT — Emit patch-ready output formats with audit hooks and schema validation. **Depends on:** v0.1.01, v0.1.03.
- ✅ (v0.1.05) Run Persistence & Artifact Store Protocols — Define storage interfaces for run state, JSONL artifacts, and logs to enable durable runs and audits. **Depends on:** v0.1.01, v0.1.02.
- ✅ (v0.1.06) Log/Event Taxonomy & Sink Protocols — Standardize run/phase event names and payloads for observability and status tooling. **Depends on:** v0.1.01, v0.1.02, v0.1.05.
- ✅ (v0.1.07) Pipeline Orchestrator Core — Orchestrate flexible phase execution with dependency gating, deterministic merges, and staleness tracking; define when phase outputs are persisted as artifacts. **Depends on:** v0.1.01, v0.1.02, v0.1.05, v0.1.06.
- ✅ (v0.1.08) Phase Execution & Sharding Config — Add per-phase execution strategies (full/scene/route) and concurrency controls. **Depends on:** v0.1.01, v0.1.07.
- ✅ (v0.1.09) Phase History & Staleness Rules — Capture phase revisions and invalidate downstream outputs when upstream changes. **Depends on:** v0.1.01, v0.1.02, v0.1.07.
- ✅ (v0.1.10) Phase Result Summaries & Metrics — Capture post-phase stats (glossary counts, QA totals, annotation coverage) for quality signals. **Depends on:** v0.1.01, v0.1.02, v0.1.07.
- ✅ (v0.1.11) CLI Workflow & Phase Selection — Provide CLI commands to run a full plan or a single phase with clear status output; wire storage adapters into CLI runs. **Depends on:** v0.1.02, v0.1.07, v0.1.10.
- ✅ (v0.1.12) BYOK Config & Endpoint Validation — Validate model endpoints and keys to avoid unsafe or unusable runs. **Depends on:** v0.1.01.
- ✅ (v0.1.13) BYOK Runtime Integration — Implement OpenAI-compatible runtime clients (pydantic-ai) with retries/backoff. **Depends on:** v0.1.12.
- ✅ (v0.1.14) Agent Runtime Scaffold (pydantic-ai) — Establish agent harness, prompts, and tool plumbing for phase agents. **Depends on:** v0.1.01, v0.1.12, v0.1.13.
- ✅ (v0.1.15) Initial Phase Agent: Context — Scene summarizer agent with TOML profile, 3-layer prompts, and strict validation. Validated with real game data. **Depends on:** v0.1.14.
- ✅ (v0.1.16) Initial Phase Agent: Pretranslation — Idiom Labeler agent that identifies idiomatic expressions, puns, wordplay, and culturally-specific phrases. Outputs `IdiomAnnotationList` with structured annotations. Validated with real LLM. **Depends on:** v0.1.14, v0.1.15.
- ✅ (v0.1.17) Initial Phase Agent: Translate — Create our main translation agent, utilizing simple direct translation. This should take in context and pretranslation data, and output translated lines. Future translation agents would involve extra features like selective usage of other MTL models as tool calls. **Depends on:** v0.1.14, v0.1.15, v0.1.16.
- ✅ (v0.1.18) Initial QA Checks (Deterministic) — Create deterministic and automated checks for formatting and completeness to enforce guardrails. **Depends on:** v0.1.01, v0.1.17.
- ✅ (v0.1.19) Initial Phase Agent: QA — Create an initial agent to act in the QA phase. Candidates include (but are not limited to): inaccurate translation detection, style-guide adherence critic, incorrect pronoun usage detection, etc. **Depends on:** v0.1.14, v0.1.18.
- ✅ (v0.1.20) Initial Phase Agent: Edit — Create our main editor agent, which will take in translated lines and apply targeted fixes based on QA findings for iterative improvements. Future editor agents would involve extra features like flagging a line for full retranslation, or using tools for more complex fixes. **Depends on:** v0.1.14, v0.1.17, v0.1.19.
- ✅ (v0.1.21) Observability Surface (CLI Status Viewer) — Display live phase status and completion summaries for trust and clarity. **Depends on:** v0.1.02, v0.1.06, v0.1.10, v0.1.11.
- (v0.1.22) Functional Onboarding — Guide users to a first successful run with defaults and validation. **Depends on:** v0.1.11, v0.1.13, v0.1.15–v0.1.20.
- ✅ (v0.1.23) Unit Test Coverage Gate (>80%) — Enforce unit test coverage threshold across core and CLI packages. **Depends on:** v0.1.01–v0.1.13.
- ✅ (v0.1.24) Integration Test Suite — Validate CLI workflows and runtime wiring across storage and BYOK endpoints. **Depends on:** v0.1.11, v0.1.12, v0.1.13, v0.1.23.
- ✅ (v0.1.25) Quality Test Suite — Real-LLM smoke tests for runtime and agent behaviors. **Depends on:** v0.1.14, v0.1.15–v0.1.20, v0.1.23.
- (v0.1.26) Standards Review: Declarative Agent Config — Lock in agent configuration conventions and documentation. **Depends on:** v0.1.14–v0.1.16.
- ✅ (v0.1.27) End-to-End Logging & Error Surfacing — Ensure full logging coverage and raise actionable errors instead of silent failures. **Depends on:** v0.1.06, v0.1.07, v0.1.14–v0.1.16.
...
**Spec List (Expanded):**
- (v0.2.01) Agent Roster & Per-Phase Composition — Let users configure which agents run in a phase and in what mix. **Depends on:** v0.1.14, v0.1.15–v0.1.20.
- (v0.2.02) Agent Pool Scheduling & Queueing — Provide async scheduling, backpressure, and sharding for multi-agent teams. **Depends on:** v0.1.07, v0.1.08, v0.2.01.
- (v0.2.03) Per-Agent Telemetry & Progress — Show which agents are running, finished, or blocked within a phase. **Depends on:** v0.1.06, v0.1.21, v0.2.01, v0.2.02.
- (v0.2.04) HITL Review & Manual Artifacts — Enable pause/review/resume with human edits without full resets. **Depends on:** v0.1.05, v0.1.07, v0.2.02.
- (v0.2.05) Deterministic Merge Policies & Conflict Resolution — Add configurable resolution rules when multiple agents overlap. **Depends on:** v0.1.07, v0.2.01.
- (v0.2.06) Incremental Rerun & Diffing — Rerun only impacted shards to reduce cost and iteration time. **Depends on:** v0.1.07, v0.1.08, v0.1.09, v0.2.02.
- (v0.2.07) Multi-Agent Orchestration — Execute multiple agents per phase with priority ordering and depends_on relationships; route summarizer runs after scene summarizer. **Depends on:** v0.1.15, v0.2.01, v0.2.02.
- (v0.2.08) Agent Hooks — Pre/post LLM generation callbacks for custom processing, validation, or logging. **Depends on:** v0.1.15.
- (v0.2.09) BatchSummarizer Agent — Context agent for content without scene boundaries; processes arbitrary line batches. **Depends on:** v0.1.15, v0.2.01.
- (v0.2.10) RouteSummarizer Agent — Context agent for route-level summaries; depends on scene summaries. **Depends on:** v0.1.15, v0.2.07.
- (v0.2.11) User-Configurable Agent Output Schemas — Allow users to define custom agent output schemas via TOML or JSON Schema without writing Python code; supports dynamic schema generation and validation. **Depends on:** v0.1.14, v0.2.01.
...
**Spec List (Expanded):**
- (v0.3.01) Multi-Language Batch Orchestration — Run multiple target languages in parallel with shared context. **Depends on:** v0.1.07, v0.1.08, v0.2.01, v0.2.02.
- (v0.3.02) Adapter Interface Framework — Standardize adapter contracts for engines and storage backends (e.g., PostgreSQL storage adapters). **Depends on:** v0.1.01, v0.1.05.
- (v0.3.03) Engine-Specific Adapters — Provide out-of-the-box adapters for popular engines. **Depends on:** v0.3.02.
...
**Spec List (Expanded):**
- (v0.4.01) Full TUI Workflow — Deliver a complete TUI for setup, progress, QA review, and edits. **Depends on:** v0.1.21, v0.2.02–v0.2.04.
...
**Spec List (Expanded):**
- (v1.0.01) CAT-Grade Features — Add TM, glossary enforcement, advanced QA suites, and reporting for v1.0 parity. **Depends on:** v0.2.01–v0.2.06.
