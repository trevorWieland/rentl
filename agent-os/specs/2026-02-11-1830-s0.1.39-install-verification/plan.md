...
- [x] Task 9: Developer verification
  ...
  - [x] Fix: Re-stabilize `test_translate_phase_produces_translated_output` in `tests/quality/pipeline/test_golden_script_pipeline.py:289`; audit round 4 reproduced full-gate regression with `make all` failing in quality (`1 failed, 11 passed`, `Failed: Timeout (>30.0s)`), while focused reruns still pass, confirming non-deterministic behavior. Keep timeout at 30s and make `make all` deterministic. (audit round 4)
  - [x] Fix: Re-stabilize `test_translate_phase_produces_translated_output` under repeated runs; audit round 5 reproduced a new intermittent failure (`exit_code: 99`) with `Agent direct_translator FAILED: Hit request limit (4)` on run 3/3 while adjacent reruns passed (failing assertion at `tests/quality/pipeline/test_golden_script_pipeline.py:317`; request-budget derivation at `packages/rentl-agents/src/rentl_agents/wiring.py:1463`). Keep timeout at 30s and make the quality gate deterministic. (audit round 5)
  - [x] Fix: Re-stabilize `test_translate_phase_produces_translated_output` under repeated runs; audit round 6 reproduced two failures in a 3-run sequence (`Failed: Timeout (>30.0s)` at `tests/quality/pipeline/test_golden_script_pipeline.py:289` and `Pipeline failed with exit code 99` at `tests/quality/pipeline/test_golden_script_pipeline.py:317` with `Agent direct_translator execution failed after 1 attempts`). Keep timeout at 30s and make the quality gate deterministic in repeated executions. (audit round 6)
...
- [ ] Task 11: Refactor init to remove provider selection and standardize env vars
  ...
  - [x] Fix: Remove cross-package import of `detect_provider` from `rentl_agents` in `packages/rentl-core/src/rentl_core/init.py:192`; `rentl-core` must not import from `rentl-agents` (violates PLC0415 lint rule, `# noqa` suppression is not acceptable). Move provider detection to the CLI layer: call `detect_provider` in `services/rentl-cli/src/rentl/main.py` and pass the provider name string to `generate_project`/`_generate_toml` via `InitAnswers` or as a parameter. Remove the `# noqa: PLC0415` suppression. (PR #123 feedback from @chatgpt-codex-connector[bot], feedback round 1)
- [x] Task 12: Fix quality tests to comply with testing standards
  ...
  - [x] Fix: Remove the manual `.env` rewrite in `tests/quality/cli/test_preset_validation.py:97-98` and run `doctor` against the `.env` generated by `rentl init` so the test validates init output directly (audit round 1; see signposts.md: Task 12, preset validation still rewrites `.env`)
  - [x] Fix: Remove remaining `.env` mutation in `tests/quality/cli/test_preset_validation.py:96-106` (`env_path.write_text(updated_env)`), since Task 12 explicitly requires removing `.env` file-writing workarounds in this test (`plan.md:96`) (audit round 2; see signposts.md: Task 12, preset validation still mutates `.env`)
- [x] Task 13: Republish all packages with Task 11 env var standardization
  - Bump all package versions from 0.1.7 to 0.1.8 (lock-step versioning)
  - Packages to bump: rentl-schemas, rentl-core, rentl-io, rentl-llm, rentl-agents, rentl
...
- [x] Task 14: Re-run demo verification with published v0.1.8
  ...
  - Update demo.md with Run 3 results
  - Test: demo Run 3 overall status is PASS
