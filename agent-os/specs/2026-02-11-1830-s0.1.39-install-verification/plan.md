...
- [ ] Task 12: Fix quality tests to comply with testing standards
  ...
  - [ ] Fix: Remove remaining `.env` mutation in `tests/quality/cli/test_preset_validation.py:96-106` (`env_path.write_text(updated_env)`), since Task 12 explicitly requires removing `.env` file-writing workarounds in this test (`plan.md:96`) (audit round 2; see signposts.md: Task 12, preset validation still mutates `.env`)
