...
This spec ensures end users can install rentl via `uvx rentl` — the primary distribution method. The package must be published to PyPI, and the full install → init → run workflow must be verified on a clean environment.

## Tasks

- [x] Task 1: Save Spec Documentation
- [ ] Task 2: Build rentl-cli package
  - Run `uv build --package rentl-cli --no-sources` from workspace root
  - Verify `services/rentl-cli/dist/` contains wheel and sdist
  - Test: build succeeds, artifacts are valid
- [ ] Task 3: Publish rentl-cli to PyPI
  - Set `UV_PUBLISH_TOKEN` from user's PYPI_TOKEN in .env
  - Run `uv publish` from `services/rentl-cli/`
  - Verify package appears on PyPI at https://pypi.org/project/rentl-cli/
  - Test: `pip show rentl-cli` works after publish
- [ ] Task 4: Verify uvx installation on fresh environment
  - On a machine without rentl installed, run `uvx rentl --version`
  - Test: version outputs correctly (e.g., `rentl v0.1.0`)
- [ ] Task 5: Verify `rentl init` end-to-end
...
- [ ] Task 6: Verify full pipeline run via uvx
