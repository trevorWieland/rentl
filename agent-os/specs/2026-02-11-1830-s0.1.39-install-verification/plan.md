...
- [ ] Task 12: Fix quality tests to comply with testing standards
  ...
  - [ ] Fix: Remove the manual `.env` rewrite in `tests/quality/cli/test_preset_validation.py:97-98` and run `doctor` against the `.env` generated by `rentl init` so the test validates init output directly (audit round 1; see signposts.md: Task 12, preset validation still rewrites `.env`)
