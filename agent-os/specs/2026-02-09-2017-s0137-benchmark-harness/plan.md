...
- [ ] Task 11: Make judge response parsing robust across model families
  ...
  - [x] Fix: Enforce structured judge output (e.g., schema-backed/pydantic-ai output) instead of text-only JSON parsing; Task 11 still sends a free-form prompt request path (`packages/rentl-core/src/rentl_core/benchmark/judge.py:255`, `packages/rentl-llm/src/rentl_llm/openai_runtime.py:72`, `packages/rentl-llm/src/rentl_llm/openai_runtime.py:96`) (audit round 1; see signposts.md Task 11)
  - [x] Fix: Remove dead `JudgeOutput` or wire it into the structured-output path; it is currently unused (`packages/rentl-core/src/rentl_core/benchmark/judge.py:25`) (audit round 1)
  - [x] Fix: Replace `Any`-typed `result_schema`/`structured_output` fields with explicit schema types in `packages/rentl-schemas/src/rentl_schemas/llm.py:6`, `packages/rentl-schemas/src/rentl_schemas/llm.py:71`, and `packages/rentl-schemas/src/rentl_schemas/llm.py:83` to satisfy `agent-os/standards/python/strict-typing-enforcement.md` (audit round 2)
  - [x] Fix: Enforce full rubric-dimension coverage in structured judge output path (require `accuracy`, `style_fidelity`, `consistency` or fail/retry) and add regression test coverage for missing-dimension structured output (`packages/rentl-core/src/rentl_core/benchmark/judge.py:279`, `tests/unit/benchmark/test_judge.py`) (audit round 2; see signposts.md Task 11 structured-output dimension completeness)
  - [ ] Fix: Align fallback text parsing with the new explicit judge JSON fields so retry/fallback mode remains functional when `structured_output` is unavailable; prompt now emits `accuracy_winner`/`style_fidelity_winner`/`consistency_winner` but parser still requires `dimension_winners`, causing `ValueError: Missing 'dimension_winners' in response` (`packages/rentl-core/src/rentl_core/benchmark/judge.py:110`, `packages/rentl-core/src/rentl_core/benchmark/judge.py:194`; repro: `python - <<'PY' ... judge._parse_head_to_head({...accuracy_winner...}) ... -> ValueError`) (audit round 3; see signposts.md Task 11 structured-output fallback mismatch)
  - [ ] Fix: Add regression test coverage for non-structured fallback parsing with explicit dimension winner fields (`tests/unit/benchmark/test_judge.py`) so Task 11 continues to pass if runtime returns text output instead of parsed schema output (audit round 3)
