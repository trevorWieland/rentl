...
- [ ] Task 8: Tests (unit + integration + quality)
  ...
  - [ ] Fix: Add missing benchmark CLI integration BDD coverage that invokes `rentl benchmark` and validates end-to-end mocked flow/report output (audit round 1; `plan.md:101`; no benchmark command invocation tests found via `rg "invoke\\(.*benchmark"` in `tests/integration`)
