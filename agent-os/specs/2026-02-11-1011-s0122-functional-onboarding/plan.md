...
- [ ] Task 2: Fix doctor `.env` loading
  ...
  - [ ] Fix: Add/move dotenv-loading tests into `tests/unit/core/test_doctor.py` (doctor-context coverage for config-dir `.env` path and `.env.local` handling); current additions were made in `tests/unit/cli/test_main.py` instead (audit round 2)
