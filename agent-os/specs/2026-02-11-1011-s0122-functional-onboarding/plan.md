...
- [ ] Task 6: End-to-end onboarding integration test
  ...
  - [ ] Fix: Replace invalid `init --target-dir` invocation in `tests/integration/cli/test_onboarding_e2e.py:100` with a supported way to run `init` in `ctx.project_dir` (e.g., chdir or isolated filesystem); current test fails with `No such option: --target-dir` and exits 2 (audit round 1)
