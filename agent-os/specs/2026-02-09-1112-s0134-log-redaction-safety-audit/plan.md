...
- [x] Task 7: Fix demo Step 5 test secret value
...
- [x] Fix Now (Spec audit round 1): Extend `check-secrets` to scan all `api_key_env` fields in `rentl.toml`, including multi-endpoint configs under `[[endpoints.endpoints]]` (not just `[endpoint]`). Evidence: scanner only checks `config_data["endpoint"].get("api_key_env")` at `services/rentl-cli/src/rentl_cli/main.py:2793`; repro with `[[endpoints.endpoints]] api_key_env="sk-1234567890abcdefghijklmnop"` returns `exit 0` / `PASS: No hardcoded secrets detected`.

- [x] Fix Now (Spec audit round 1): Replace `.gitignore` substring matching with actual ignore-rule evaluation for `.env` (for example `git check-ignore .env` fallback) and add regression coverage. Current check `if ".env" not in gitignore_contents` at `services/rentl-cli/src/rentl_cli/main.py:2841` and `services/rentl-cli/src/rentl_cli/main.py:2861` falsely treats `.env.example` as sufficient; repro in a git repo with `.env` present and `.gitignore` containing only `.env.example` returns `exit 0` / `PASS: No hardcoded secrets detected`.
