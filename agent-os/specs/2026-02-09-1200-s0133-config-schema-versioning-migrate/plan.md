...
- [x] Task 7: Fix Doctor Auto-Migration
  ...
  - Option 1: Extract migration logic from CLI's `_auto_migrate_if_needed` to a shared core function that both CLI and doctor can use
  - Option 2: Make doctor accept a pre-loaded config from CLI (breaking change to API)
  - Option 3: Import and use CLI's migration function from core (creates circular dependency)
  ...
  - [x] Fix: Add the missing integration coverage in `tests/integration/core/test_doctor.py` for an outdated config (`0.0.1`) that verifies doctor auto-migrates, writes `rentl.toml.bak`, and then passes validation; current proof exists only as a unit test in `tests/unit/core/test_doctor.py:283` (audit round 1)
