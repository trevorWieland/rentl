...
- **Task:** Task 9 (gate triage round 2)
- **Status:** resolved
- **Problem:** Verified-model overrides drifted from live provider behavior: local `google/gemma-3-27b` and `qwen/qwen3.5-35b-a3b` fail context with `Exceeded maximum retries (2)`, `openai/gpt-oss-120b` fails qa with `Exceeded maximum retries (1)`, and `z-ai/glm-5` pretranslation returns `400 Tool choice must be auto` because registry left it at default `supports_tool_choice_required=true`.
- **Evidence:** Gate output reports `Exceeded maximum retries (2)` for local context phases and `Exceeded maximum retries (1)` for `gpt-oss-120b` qa at `tests/quality/compatibility/test_model_compatibility.py:121`. `z-ai/glm-5` returns `400 Tool choice must be auto` on pretranslation.
- **Evidence:** Registry had uniform `timeout_s=8.0`, `max_output_retries=2` for local models and `timeout_s=10.0`, `max_output_retries=1` for OpenRouter models with no `supports_tool_choice_required` override for `z-ai/glm-5`.
