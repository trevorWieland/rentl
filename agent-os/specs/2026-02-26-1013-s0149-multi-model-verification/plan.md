...
- [ ] Task 7: Resource-aware LM Studio model lifecycle management
  ...
  - [ ] Fix: Make `load_lm_studio_model` fail-fast when `list_lm_studio_models` fails instead of proceeding to a blind load; this branch currently bypasses pre-unload and can violate single-model residency acceptance (`packages/rentl-core/src/rentl_core/compatibility/loader.py:149-158`) (audit round 3)
  - [ ] Fix: Replace `test_load_model_proceeds_when_list_fails` with assertions that list failure returns `ModelLoadError` and does not POST `/load`, so tests enforce the no-multiple-loaded-models contract (`tests/unit/core/compatibility/test_loader.py:217-242`) (audit round 3)
