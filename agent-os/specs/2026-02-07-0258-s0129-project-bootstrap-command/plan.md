...
- [ ] Task 7: Add missing ingest and export phases to generated pipeline config
  ...
  - [ ] Fix: Patch the deterministic test double at the execution boundary actually used by `run-pipeline` (agent execution via `build_agent_pools()`), because patching `cli_main._build_llm_runtime` does not affect this path; currently `test_init_produces_runnable_project` fails with `exit code 99` / `Agent scene_summarizer execution failed after 4 attempts` (`tests/integration/cli/test_init.py:264`, `tests/integration/conftest.py:149`, `services/rentl-cli/src/rentl_cli/main.py:1404`) (audit round 5)
  - [ ] Fix: Add a regression assertion that the patched execution boundary was invoked during `run-pipeline` and keep the export-artifact assertions (`out/{lang}/output.{format}`) gated on a successful `exit_code == 0` run (`tests/integration/cli/test_init.py:257`, `tests/integration/cli/test_init.py:281`) (audit round 5)
