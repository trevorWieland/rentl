...
- [ ] Task 7: Add missing ingest and export phases to generated pipeline config
  ...
  - [ ] Fix: Restore true end-to-end execution coverage in the BDD step by invoking `rentl run-pipeline` (or equivalent orchestrator execution path) and asserting successful completion; the current step labeled "execute end-to-end" only validates config + `build_agent_pools()` (`tests/integration/cli/test_init.py:194`, `tests/integration/cli/test_init.py:233`) (audit round 4)
  - [ ] Fix: Add explicit assertions for generated export artifacts after execution (for example `out/{target_language}/output.{format}`); current Task 7 step checks only directory existence and never verifies export outputs (`tests/integration/cli/test_init.py:246`, `tests/integration/cli/test_init.py:259`) (audit round 4)
