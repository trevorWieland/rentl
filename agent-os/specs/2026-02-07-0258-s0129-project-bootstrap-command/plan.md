...
- [ ] Task 4: Add `rentl init` CLI command
  ...
  - [ ] Fix: Preserve `typer.Exit` passthrough in `init()` so overwrite cancellation exits cleanly instead of entering `_error_from_exception()` (`services/rentl-cli/src/rentl_cli/main.py:211`, `services/rentl-cli/src/rentl_cli/main.py:290`) (audit round 1)
  - [ ] Fix: Add CLI unit tests for `rentl init` happy path and overwrite confirmation/cancel flow (including cancellation exit code assertions) (`tests/unit/cli/test_main.py`) (audit round 1)
