...
- **Task:** Task 7 (Full Pipeline Smoke Test) - BLOCKED
...
- **Impact:** Cannot complete Task 7 acceptance criteria ("all phases complete successfully") without either: (1) modifying `build_agent_pools` to accept injectable runtime, (2) implementing HTTP mocking at httpx level to intercept pydantic-ai calls, or (3) running actual mock HTTP server on localhost:8001. Current FakeLlmRuntime pattern used successfully in `tests/integration/conftest.py:23` only works for direct `_build_llm_runtime()` consumers but not for agent pool execution.
- **Files affected:** `tests/integration/pipeline/test_golden_script_pipeline.py`, `tests/integration/conftest.py`, `packages/rentl-agents/src/rentl_agents/wiring.py`, `packages/rentl-llm/src/rentl_llm/openai_runtime.py`
