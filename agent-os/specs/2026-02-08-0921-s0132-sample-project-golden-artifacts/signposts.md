...
- **Problem:** Task implementation drifted from the recorded contract: spec/standards require an integration-layer full pipeline test with FakeLlmRuntime, but the current implementation is quality-layer with real runtime and hard-coded endpoint env requirements.
- **Evidence:** Contract still specifies integration + FakeLlmRuntime at `agent-os/specs/2026-02-08-0921-s0132-sample-project-golden-artifacts/spec.md:35` and pipeline tests in integration at `agent-os/specs/2026-02-08-0921-s0132-sample-project-golden-artifacts/standards.md:7`; implementation is at `tests/quality/pipeline/test_golden_script_pipeline.py:1` with `api_key_env = "PRIMARY_KEY"` at `tests/quality/pipeline/test_golden_script_pipeline.py:68`. Running `uv run pytest -q tests/quality/pipeline/test_golden_script_pipeline.py --timeout=30` fails: `config_error` / `Missing API key environment variable: PRIMARY_KEY`.
- **Impact:** Future Task 7 attempts can appear complete in plan text while still violating the spec/standards contract and producing a smoke test that fails in default environments before any pipeline phase runs.
