...
- [ ] Task 7: Full Pipeline Smoke Test
  ...
  - [ ] Fix: Resolve contract mismatch by implementing runtime injection for agent pools (or equivalent interception) so a full-pipeline test can run under `tests/integration/` with FakeLlmRuntime per `agent-os/specs/2026-02-08-0921-s0132-sample-project-golden-artifacts/spec.md:35` and `agent-os/specs/2026-02-08-0921-s0132-sample-project-golden-artifacts/standards.md:7`; current wiring path (`services/rentl-cli/src/rentl_cli/main.py:1684` → `packages/rentl-agents/src/rentl_agents/wiring.py:1108`) still has no injectable runtime seam (audit round 3)
  - [ ] Fix: Make Task 7 pass reliably in the full gate by removing the hard dependency on an external endpoint for `tests/quality/pipeline/test_golden_script_pipeline.py`; current run `uv run pytest -q tests/quality/pipeline/test_golden_script_pipeline.py --maxfail=1` fails at `tests/quality/pipeline/test_golden_script_pipeline.py:197` with `runtime_error: Agent pool task failed: Agent scene_summarizer execution failed after 2 attempts` / `Connection error.` (audit round 3)
