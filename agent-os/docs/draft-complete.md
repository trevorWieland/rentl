...
A spec-driven development workflow powered by AI agents and an automated orchestrator. Agent OS coordinates planning, implementation, auditing, and demo validation through a pipeline of specialized agent commands.
...
All agent commands live in `.claude/commands/agent-os/`. The orchestrator script lives at `agent-os/scripts/orchestrate.sh`. Standards live in `agent-os/standards/`. Product documentation lives in `agent-os/product/`. Specs live in `agent-os/specs/`.
...
3. Align with product context (from `agent-os/product/`).
4. Select applicable standards (from `agent-os/standards/index.yml`).
...
1. **Task loop**: For each unchecked task in plan.md:
   ...
   - Run `make check` (task gate). If it fails, re-invoke `do-task` with the error output up to 3 times.
   - Invoke `audit-task` to audit. If it finds issues, it unchecks the task and adds fix items; the loop picks it up again.
2. **Spec gate**: Run `make all`. If it fails, invoke `do-task` to fix.
...
2. Runs `make all` to confirm everything passes.
...
1. **Resolve the Spec Issue.** Three paths:
   ...
   - Pick the next best issue from the roadmap: run `python3 agent-os/scripts/list-candidates.py` (resolves dependency chains, shows only earliest milestone). Present candidates to the user. Once chosen, fetch with `gh issue view`.
...
6. **Standards.** Read `agent-os/standards/index.yml` and propose relevant standards.
...
5. **Run Task Gate.** Run `make check`. If it fails, read errors, fix, and re-run until green. If stuck after reasonable attempts, write a signpost with exact error output and exit as blocked.
...
**Suggested model:** Different model from do-task for independence (e.g., Codex medium reasoning).
...
**Suggested model:** Strong reasoner, different from do-task (e.g., Codex high reasoning). Quality matters more than speed.
...
3. **Run Verification Gate.** `make all`. If fails, stop and report.
...
7. **Update Roadmap.** If the spec has an entry in `agent-os/product/roadmap.md`, prefix with checkmark. Commit on the branch.
...
A config file is sourced as a bash file, so it can set any of these variables.
...
4. Run the task gate (`make check`). If it fails, re-invoke do-task with the gate error output. Retry up to `MAX_GATE_RETRIES` (hardcoded to 3) times.
...
**Phase 2: Spec Gate.** Run `make all`. If it fails, invoke `do-task` with the gate error output and restart the cycle.
...
When `make check` fails after a do-task invocation:
...
5. For Codex agents: use `-o` flag to capture last message to a file.
6. For Claude agents: capture stdout to a temp file.
...
- Victory fanfare (audio) on success (best-effort, using paplay, aplay, mpv, ffplay, or PowerShell on WSL).
...
6. **Run Gate (If Code Changes).** `make check`. If non-trivial fix needed, add task to plan.md.
...
**Purpose:** Establish foundational product documentation through interactive conversation.
...
**Purpose:** Extract tribal knowledge from the codebase into concise, documented standards.
...
| Gate | Command | What it runs | When used |
|------|---------|--------------|-----------|
...
**Belt-and-suspenders:** The agent runs the gate internally (do-task runs `make check`), and the orchestrator re-runs it as a hard external gate. This catches cases where the agent claims success but the gate actually fails.
...
A core design principle: implementation and auditing use different AI models/providers to avoid "grading your own homework." Default configuration:
- Implementation (do-task, run-demo): Claude Sonnet.
- Auditing (audit-task, audit-spec): Codex.
...
Using the same model to both write and review code is like grading your own homework. The orchestrator defaults to different model providers for implementation (Claude Sonnet) and auditing (Codex) to ensure independent review.

### Belt-and-Suspenders Verification

Agents run verification gates internally (do-task runs `make check`), and the orchestrator re-runs them externally as hard gates. This catches cases where agents claim success but the gate actually fails.
